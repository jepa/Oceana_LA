...
```{r SAU_Landings_Data, eval = T, echo = F, warning =F, message = F, fig.height = 8, fig.width= 12, fig.cap= "Top 10 species by landings from the SAU dataset. Brown bars represent species not present in the other category. White boxes represent the contribution of that speciest to the total country landing"}
...
```{r SIGIF_Map, eval = T, echo = F, warning =F, message = F}
...
```{r Comercial_Plot, eval = T, echo = F, warning =F, message = F, fig.height=6, fig.width= 7, fig.cap= "Quantity of fish and seafood products commercialized per month in 2017 at CEAGESP market (Source: CEAGESP)"}
...
```{r top_fisheries_catch, eval = T, echo= FALSE, warning=FALSE,fig.height=6, fig.width= 7}
...
```{r top_fisheries_aqua, eval = T, echo= FALSE, warning=FALSE,fig.height=6, fig.width= 7}
