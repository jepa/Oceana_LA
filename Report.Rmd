...
3. A recent study estimated the proportion of fish landings destinated to human direct consumption and fish meal and fish oil[@Cashion:2018]. Such study used the SAU database, internal reports and other methods for estimating total landings devoted to each segment for all countries in the world.

### Analysis

For the former analysis we used the SAU database to estimate the total landings (tons) of Chile, Brasil, and Peru in 2014, the last year of data in the SAU database. All fishing sectors were aggregated (Industrial, Subsistence and Artisanal) and ** COMMENT: "Recreational" was ignored as it does not contribute to local food security.** Other countries that fish within Chile, Peru and Brasil were also not included in the analysis for the same reason. Landings were grouped by taxa excluding those not identified (e.g. "Marine fishes not identified"). We used the same database and method to estimate the revenue (US dollars) from each fishery.The SAU landings database reconstruction methods for each country can be accessed for Brazil[@], Chile[@], and Peru[@]. 

We used the SAU information on economic value of fish lansings as published in the website. Fishing revenue integrates the different ex-vessel price that landings have, depending on the final destination: direct human consumption (DHC) and indirect human consumption (IHC). Therefore, total revenue (*R*) is estimated in the SAU database as follows:

$$R = \sum_{i = 1}^n (L_{i}*ß_{i})*ßP_{i} + (L_{i}*∂_{i}) *∂P_{i}$$
where *L* is total landings for species *i*, *ß* is the proportion of total landings destined to DHC, ßP s the ex-vessel price for DHC, *∂* is the proportion of total landings destined to IHC and ∂P is the ex-vessel price for it.
