...
```{r UN_Landings_Data, eval = T, echo = T}
...
Totals <- Clean_FAO %>% 
  group_by(Country,
           ...
           Unit) %>% 
  summarise_at(vars(matches("X20")), sum, na.rm=T) %>% 
  rowwise() %>% #DPLYR FUNCTION FOR rowMeans!!!!
  mutate(Mean = mean(c(X2014,X2015,X2016))) %>% 
  filter(Unit != "Number",
         Sector != "Trade" | Trade.Flow != "Production"
  )
  ...
  if( Var == "Tonnes"){
    ...
    Result <- Spp_Data %>%
      ...
      mutate(
        ...
        DHC_Available = ((Catch * DHC) + Aquaculture + Imports) - Exports,
        Catch_IHC = (Catch * (1-DHC))*-1
      ) %>% 
      group_by(Country) %>% 
