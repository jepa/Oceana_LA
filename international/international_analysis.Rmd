...
Proportions <- SAU_Database %>%
  spread(Variable,Value) %>% 
  group_by(scientific_name,Reporter) %>% 
  summarise_at(vars(Landings_Value:Total_tonnes), mean,na.rm=T) %>% 
  left_join(Totals,
            by = "Reporter") %>% 
  select(-`Exploited Species`) %>% 
  mutate(
    Proportion_Landings = round(Landings_Value/`Total Revenue`,2),
    Proportion_Revenue = round(Total_tonnes/`Total Landings`,2)
  ) %>% 
  group_by(Reporter) %>% 
  summarise(
    Top = sum(Proportion_Landings),
    N = n()
  )
