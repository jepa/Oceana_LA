...
#read dataset
FAO <- read.delim(paste(Data_Path,FAO_F_Name,
                      sep = ""))
...
# File name
A_F_Name <- "INPUT Matching Names_B.csv"
#read dataset
...
Clean_FAO_Spp <- Clean_FAO %>% 
  group_by(Comm.Name) %>% 
  summarise(n=n()) %>% 
  select(-n)


First_FAO_Andres <- Clean_FAO_Spp %>% 
  rename(FishStatName=Comm.Name) %>% 
  left_join(Andres_Names,
            by = "FishStatName")

Second_FAO_Andres <- First_FAO_Andres %>% 
  filter(is.na(Taxon.Key)) %>% #Those not identified in the first column
  select(1) %>% 
  rename(FishStatName2=FishStatName) %>% 
  left_join(Andres_Names,
            by = "FishStatName2")
  
  
Final_FAO_Andres <-First_FAO_Andres %>% 
  bind_rows(Second_FAO_Andres) %>% 
  rename(Comm.Name=FishStatName)

Clean_FAO_Andres <- Clean_FAO %>% 
  left_join(Final_FAO_Andres,
            by = "Comm.Name")
  
x <- Clean_FAO_Andres %>% 
  filter(is.na(Taxon.Key)) %>% 
  semi_join(Clean_FAO,
            by = "Comm.Name") %>% 
  group_by(Comm.Name) %>% 
  summarise(n())
...
Tim_Spp$taxon_name <- gsub(" ", ".",Tim_Spp$taxon_name)

Tim_Spp_Andres <- Tim_Spp %>% 
  rename(FishStatName2 = taxon_name) %>% 
  left_join(Andres_Names,
            by = "FishStatName2")
...
Clean_Tim <- Tim_D %>% 
  ...
  group_by(
           ...
           MatchName,
           MatchGroup) %>% 
  summarise_at(c("DHC_Use","FMFO_Use","Other_Use","IHC"),mean,na.rm=T)


#### Merge with Sofia-Andres ####


Taxons_Semi <- Sofia_Andres %>% 
  semi_join(Clean_Tim,
            by = "Taxon.Key") %>% 
  group_by(Common_Name) %>% 
  summarise(n())

Taxons_Anti <- Sofia_Andres %>% 
  anti_join(Clean_Tim,
            by = "Taxon.Key") %>% 
  group_by(Common_Name) %>% 
  summarise(n())

Merge_Andre_Tim <- Sofia_Andres %>% 
  left_join(Clean_Tim,
            by = c(
              "Taxon.Key",
              "Common_Name",
...
Coor <- read.csv("/Users/jpalacios/Documents/UBC/Oceans_Project/Distribution/DBEM/Data/Lon_Lat_DBEM.txt")
...
SAU_Coor <- SAU_Spatial %>%
  ...
  left_join(Coor,
            by = "cell_id") %>% 
  mutate(
    Territory1 = ifelse(fishing_entity_id == 15, "Brazil",
